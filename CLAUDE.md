...
**Service catalogue (P39a, 2026-09-13)** — `lib/serviceCatalog.js` is the curated "most common services" list per vertical (names, typical minutes, `kind: 'class'` for fitness). `GET /api/cms/service-catalog?siteId=` (owner JWT + ownership) returns it plus `community` = names other shops in the same vertical typed at least twice (`service_suggestions`, migration `service_suggestions_v1.sql`, server-role writes, staff read). `POST /api/cms/service-catalog/suggest` records a custom name. Prices are never in the catalogue (they differ per market). Promote frequent suggestions into the file by hand; keep names plain and vertical-neutral.
