...
**Brand domains go through `lib/tenantHosts.js` (2026-09-16).** ONE policy module decides how a tenant's own domain is served: `hostname` (Cloudflare for SaaS Custom Hostname on the stemfra.com zone, apex + www; a BYO owner CNAMEs at `TENANT_CNAME_TARGET` = `sites.stemfra.com`, and for a domain whose zone we run the module writes that CNAME itself, DNS-only) or `pages` (the legacy Pages attach while `TENANT_CUSTOM_HOSTNAMES` is false). NEVER route the Worker on a tenant's own zone: `wrangler deploy` deletes routes it does not know on every zone (it took cleancutsbarber.click down for two minutes on 2026-09-16). `attachBrandDomain` / `brandDomainStatus` / `detachBrandDomain` / `cnameTargetFor` are the only calls the controllers, `attachSiteDomain` and `domainPurchase` make; never call `cloudflarePages.attachCustomDomain` for a brand domain again. Raw API in `lib/cloudflareCustomHostnames.js` (1404/1456 → `SaasNotEnabledError`). Setup + migration: `scripts/setup-custom-hostnames.js` (dry-run default). Runbook, table and rollback: `docs/DOMAINS.md` §7.
...
**Call transcripts (`lib/callTranscripts.js`, 2026-09-11)** — `/recording-status` queues `transcribeCall(callId)` for recordings of 20 s or more: the server fetches the mp3 from Twilio (production only, never from a Mac), sends it to OpenAI transcription (`gpt-4o-mini-transcribe`, 25 MB cap, `prompt` carries the business name and "Stemfra"), summarises with `gpt-4o-mini` into `{summary, questions, objections, agreed, next_step, sentiment, coaching}` and stores `calls.transcript / transcript_summary / transcribed_at / transcript_error` (migration `call_transcripts_v1.sql`, applied). `POST /api/twilio/recording/:callId/transcribe` (staff JWT, `{force}`) backfills older calls from the CRM drawer's Activity panel. Env in `.env.example`. Do not add a client-side transcription path.
...
**Service catalogue (P39a, 2026-09-13)** — `lib/serviceCatalog.js` `catalogFor(vertical)` (async, 5-minute cache) returns the vertical's **seed site's active services** (`verticalConfig.seedSite`, names + minutes + `kind` as the seed has them), which is the wizard's tick list. Peter's rule (same day): the seed lists are the curated list; no second hand-written catalogue (it produced near-duplicates). `GET /api/cms/service-catalog?siteId=` (owner JWT + ownership) returns it plus `community` = names other shops in the same vertical typed at least twice (`service_suggestions`, migration `service_suggestions_v1.sql`, server-role writes, staff read). `POST /api/cms/service-catalog/suggest` records a custom name. To promote a suggestion, add it to the seed site's services in the CMS.

**Lead-gen run feedback (2026-09-13)** — `POST /api/leadgen/run-complete` (secret-gated by `x-leadgen-secret`, no user JWT) is where the n8n workflow's v14 "Run Summary" node reports every run's outcome, whatever stage it stopped at: it closes the `leadgen_runs` row (status completed | empty | failed, `leads_found`, `notes` = the one-sentence summary, `metadata` = the counts) and rings the requester's bell (`crm_notify`, kind `leadgen_run`, route `/leads`). `/trigger` relays the same summary in its own response when n8n answers inside the 25 s wait (fast early exits) and then the callback skips the bell (`answeredInline`). n8n side: `n8n-workflows/leadgen-run-feedback-v14.paste.md` (If nodes at the website filter, dedupe and score gate, link-in-bio hosts count as "no website", `leads.leadgen_run_id` stamped at insert). Peter's rule: a run that stops early must say so.

**Lead-gen batches + the old-school exclusion (2026-09-16)** — `lib/leadgenRun.js startRun(user, body)` is the trigger's body (the route is a thin wrapper); `lib/leadgenBatch.js` runs a list of city × vertical runs back to back on the server (`POST /api/leadgen/batch`, `GET /batch/status`, `POST /batch/cancel`; waits for each run's `/run-complete` before the next; one bell with totals per country). Plans: `scripts/leadgen-batch-plans/`. `readiness = old_school` listings never enter the CRM: n8n `Digital-ready?` gate (v15 import built by `n8n-workflows/make-v15.js`) + DB trigger `leads_reject_old_school` (backstop). Doc: `docs/LEADGEN.md` "Batch runs" + "Digital readiness".

**Native lead-gen (P41, 2026-09-17)** — cold runs run IN this server by default: `lib/leadgenNative.js` (async Apify scrape → website gate → normalize → digital-ready gate → dedupe → OpenAI JSON scoring + A1 draft → insert → `closeRun`). `lib/leadgenRun.js` picks the engine (`LEADGEN_ENGINE` native | n8n, per-request `engine`, automatic n8n fallback when `APIFY_TOKEN` / `OPENAI_API_KEY` are missing) and owns `closeRun` (also behind `/run-complete`). Scoring rules live ONLY in `prompts/leadgen-system.txt` (loader rejects retired pricing); review count, rating, booking platform and `trait_volume` are computed in code, never by the model. Every scraped place leaves a decision in `leadgen_runs.metadata.decisions`. Test: `node scripts/leadgen-native-test.js` (dry run default, `--apply` inserts). Read `docs/LEADGEN_NATIVE.md` before touching it.

**Find my business on Google (P39, 2026-09-14)** — `lib/googlePlacesFinder.js` runs one Apify Google Maps Scraper search synchronously (`APIFY_TOKEN`, `compass~crawler-google-places` run-sync, 4 places, no paid add-ons, 90 s cap) and shapes candidates for the wizard: name, address parts, phone, website, category, rating, claimed, description, socials Google lists, `openingHours` raw plus `businessHours` (our HoursJson; `hoursFromGoogle` handles "10 AM to 8 PM", "9 to 5 PM", "Closed", "Open 24 hours", split shifts keep first open / last close). `POST /api/cms/google-profile/find {siteId, name, where}` (owner JWT + ownership) and `POST /use {siteId, candidate}` (writes `sites.business_hours`, empty social handle columns, and `site_theme_settings.metadata.gbp = {has_profile:'yes', profile_url, place_id, snapshot, found_at}`). Facts only, same rule as the lead snapshot.

**Lead Google refresh (2026-09-15)** — `lib/leadGoogleRefresh.js` refills a lead's `raw_signal` from the same Apify search the wizard finder uses (`googlePlacesFinder.searchPlaces`, the raw items): `refreshLeadFromGoogle(leadId, {dryRun, actor})` searches the company name near `<city from the stub>, <region>`, takes the place whose name (else phone) matches, stores the v13 record (`recordFromPlace`, the n8n Normalize shape) + `readiness` / `qualification.readiness_signals` (`readinessOf`, the v13 rule), fills an empty `phone` / `source_detail`, logs `lead_google_refreshed`; unmatched leads are left untouched. `startSweep({limit, dryRun, requestedBy})` runs every thin lead (`isThin`: no record, under 200 bytes or no `placeId`; test leads skipped) one at a time in the background with 1.5 s between lookups and bells the requester via `crm_notify` when done; `sweepStatus()` is the in-memory progress. Routes `routes/admin/leads.js` (`/api/admin/leads`): `POST /:id/google-refresh` (staff), `POST /google-refresh/sweep` (PLATFORM_OPS), `GET /google-refresh/status`. Not Twilio; Apify only.

**Solo shop (P39b, 2026-09-13)** — `POST /api/cms/team/solo {siteId}` (`routes/cms/team.js`, owner JWT + ownership) is the wizard's "Just me": deletes the site's team rows, links and availability rules, inserts one owner record (name from `contacts` first/last, else the company name; `is_owner`, `metadata.solo = true`), links it to every active service and writes `weekly_recurring` availability from `sites.business_hours` (0 = Sunday). Templates hide the team while that record is the only member (site-data `visibleTeam`). `siteCompleteness` "at least one team member" stays satisfied. Never call it on a site with real bookings (it deletes members).

**New site with copied parts (2026-09-12)** — `POST /api/cms/sites` accepts `copyFromSiteId` + `parts[]` (`SITE_PARTS`: brand | services | team | hours | content). The site is provisioned from the vertical seed as always; then `copySiteParts(source, new, parts, { renameTo })` (lib/provisionSite) deletes the seeded rows of each ticked part and copies the owner's rows (ids remapped, team↔service links only when both team and services are copied, brand = theme settings + template_id when the vertical matches, hours = business_hours + time_zone), renaming the source brand in text. Ownership of the source is verified; a copy failure logs and leaves the seeded site rather than failing the create. Response carries `copied[]`.
...
**`leads.readiness` (2026-09-11)** — modern | middle | old_school, set by the n8n Normalize/Parse v13 pastes from the Google Maps record (old_school = listing never claimed; middle = claimed only; modern = claimed + owner description/post, 20+ photos, booking link or social). Signals in `qualification.readiness_signals`. Owner replies and review recency are deliberately not factors; business age is a call question (first-review date needs the paid reviews fetch). Migration `leads_readiness_v1.sql` applied. Rule: the tier is computed once at ingest from the scrape, never edited by hand in the CRM.

**Walk-ins carry no commission (P36, 2026-09-11)** — `site_bookings.source` (web | chat | voice | owner_walk_in | owner_phone | import; migration `site_bookings_source_v1.sql` applied) is set ONCE by the creating code path: `placeBooking` / `placeBookingGroup` / `bookClassSession` take `source` (default `web`; `frontdeskBooking` passes `chat`), the CMS manual booking writes `owner_walk_in` or `owner_phone`. `reportsController.classify` returns kind `walkIn` for owner_* rows (counted as `walkInCents/Count`, NOT in `totalCents`/at-visit), `commissionMeter` therefore excludes them (line item carries `walk_in_cents` for transparency) and `bookingAutoCollectSweeper` skips them. Fees page + concierge context say so. Never let the source be edited after creation, and never treat a NULL source as owner-made.
