...
async function meterSiteCommission(siteId, period, { dryRun = false } = {}) {
  ...
  // No sales, no invoice (2026-09-16): a $0 statement is not a charge, and an
  // open $0 row would be dunned as "past due" (it happened to stemfra-support).
  // A $0 DUE row left from an earlier run is voided rather than updated.
  if (amountCents === 0) {
    const { data: zero } = await supabase
      .from('billing_charges')
      .select('id, status')
      .eq('site_id', siteId).eq('kind', 'commission').eq('period_start', periodStart)
      .maybeSingle();
    if (zero && zero.status === 'due') {
      await supabase.from('billing_charges').update({ status: 'void', note: 'No sales in the period: nothing to bill.' }).eq('id', zero.id);
      return { ...result, chargeId: zero.id, voided: true, skipped: 'zero_gmv' };
    }
    return { ...result, chargeId: zero?.id || null, skipped: 'zero_gmv' };
  }
