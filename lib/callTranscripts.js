...
// The gpt-4o transcribe models stop writing at about 2,000 output tokens, which
// cut a 16-minute call (Neil's Barbershop, 2026-09-11) to its first ~8,400
// characters. whisper-1 has no such cap (only the 25 MB file limit), so long
// recordings go there; short calls keep the better spelling of the 4o model.
const LONG_CALL_SECONDS = Number(process.env.TRANSCRIBE_LONG_SECONDS || 480);
const LONG_MODEL = process.env.TRANSCRIBE_LONG_MODEL || 'whisper-1';
...
async function transcribeCall(callId, { force = false } = {}) {
  ...
  try {
    ...
    const model = seconds > LONG_CALL_SECONDS ? LONG_MODEL : TRANSCRIBE_MODEL;
