...
async function buildOnboardingChecklist(siteId) {
  ...
  const GOAL_STEPS = {
    ...
    brand: ['logo'],
    // 'winback' replaced 'payments' at signup (2026-09-14): the review link
    // feeds the review-request email, the first win-back lever.
    winback: ['review_link', 'social'],
    payments: [],
  };
  ...
  const steps = [
    ...
    // The setup wizard as ONE row at the end, not Stacy's focus (Peter,
    // 2026-09-14): four screens (business, services, team, branding), then
    // preview and publish. Done once the wizard has been finished or dismissed.
    { key: 'setup_wizard', kind: 'fill', label: 'Run the setup wizard', hint: 'Business details, services, team and branding in four screens, then preview and publish. Run it again any time to update the basics.', route: CMS_ROUTES.setupWizard, auto: meta.setup_wizard_dismissed === true },
  ];
