...
function emailAllowed(lead) {
  // CANADA (CASL, 2026-09-17, Peter: "if it can raise a legal claim, we do not send"). CASL is
  // opt-in and the burden of proof is on the sender. A scraped address qualifies only under
  // "conspicuous publication" (published by the business itself, no "no solicitation" notice
  // beside it, message relevant to their role) and we must be able to PROVE it per message.
  // Lead-gen does not record where an address was published, so a cold email to a Canadian
  // lead is refused unless a basis exists: they wrote to us first (any inbound source or a
  // reply), they are a client, or a rep recorded the basis on the lead
  // (qualification.casl_basis = where the address is published, checked for a notice).
  if (marketFor(lead) === 'CA') {
    const q = (lead && lead.qualification) || {};
    const inbound = lead?.source && !['google_maps', 'import', 'manual'].includes(lead.source);
    if (inbound || lead?.outreach_reply_text || lead?.stage === 'won' || (q.casl_basis && String(q.casl_basis).trim())) return { ok: true };
    return { ok: false, reason: 'casl_no_basis', message: 'Canada (CASL): no cold email without a recorded basis. Phone first; email once they reply or agree, or record where their address is published (no "no solicitation" notice) as the CASL basis on the lead.' };
  }
