...
const { locationForRequest } = require('./ipLocation');
...
async function notifySecurityEvent({ authUserId, email, kind, req, newEmail = null, deviceUa = null }) {
  ...
  const location = await locationForRequest(req, ip); // "Brooklyn, New York, United States" or null
  ...
  if (siteIds.length) {
    await supabase.from('cms_notifications').insert(siteIds.map((site_id) => ({
      ...
      title: copy.subject,
      body: [device, location ? `near ${location}` : null, ip ? `IP ${ip}` : null, when].filter(Boolean).join(' · '),
      href: '/profile/security', metadata: { kind, device, ip, location },
  ...
  for (const to of new Set(recipients)) {
    const mail = tx.accountSecurity({ kind, firstName, email, whenLabel: when, device, ip, location, newEmail, securityUrl: `${CMS_URL}/profile/security` });
