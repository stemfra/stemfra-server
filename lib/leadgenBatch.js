...
// A 100-place run scores every digital-ready candidate one by one; New York
// salons took 9 min for ~80 candidates and massage ran past 12 min, so the
// wait is generous. Moving on early starts a second n8n execution beside the
// first and slows both (2026-09-16).
const RUN_TIMEOUT_MS = Number(process.env.LEADGEN_BATCH_RUN_TIMEOUT_MS || 30 * 60_000);
...
// Durable copy in crm_settings (key 'leadgen_batch'): a deploy restarts the
// process (a roadmap push killed the first batch after 4 of 18 runs on
// 2026-09-16), so the state is written after every change and resumeOnBoot()
// picks an unfinished batch up where it stopped.
const SETTINGS_KEY = 'leadgen_batch';
async function persist() {
  if (!batch) return;
  const { cancelled, ...rest } = batch;
  await supabase.from('crm_settings').upsert({ key: SETTINGS_KEY, value: { ...rest, cancelled: !!cancelled }, updated_at: new Date().toISOString() }, { onConflict: 'key' })
    .then(({ error }) => { if (error) console.error('[leadgen-batch] persist failed:', error.message); });
}
...
async function settleFromRow(r, row) {
  r.status = row.status;
  r.inserted = row.leads_found || row.metadata?.inserted || 0;
  r.message = row.notes || null;
}

async function runBatch(startIndex = 0) {
  for (batch.index = startIndex; batch.index < batch.runs.length; batch.index++) {
    if (batch.cancelled) break;
    const r = batch.runs[batch.index];
    if (!OPEN.has(r.status)) continue; // already settled (a resumed batch)
    try {
      if (r.status === 'running' && r.runId) {
        // Resumed mid-run: n8n is (or was) still working on it; just wait for the row.
        const row = await waitForRun(r.runId);
        if (row) await settleFromRow(r, row); else { r.status = 'timeout'; r.message = 'n8n did not report within the wait'; }
      } else {
        r.status = 'running';
        await persist();
        ...
        r.runId = out.runId;
        await persist();
        if (!out.json.success) { r.status = 'failed'; r.message = out.json.message; }
        else if (out.json.summary) {
...
function startBatch({ requestedBy, runs, paceSeconds = 20 }) {
  ...
  persist().then(() => runBatch()).catch((e) => { console.error('[leadgen-batch] crashed:', e.message); batch.finishedAt = new Date().toISOString(); persist(); });
  return batchStatus();
}
...
/** On boot: pick up an unfinished batch from crm_settings (a deploy restarted us). */
async function resumeOnBoot() {
  try {
    const { data } = await supabase.from('crm_settings').select('value').eq('key', SETTINGS_KEY).maybeSingle();
    const saved = data?.value;
    if (!saved || saved.finishedAt || saved.cancelled) return null;
    batch = { ...saved, cancelled: false };
    const startIndex = batch.runs.findIndex((r) => OPEN.has(r.status));
    if (startIndex < 0) { batch.finishedAt = new Date().toISOString(); await persist(); return null; }
    console.log(`[leadgen-batch] resuming ${batch.id} at run ${startIndex + 1} of ${batch.runs.length}`);
    runBatch(startIndex).catch((e) => { console.error('[leadgen-batch] resumed batch crashed:', e.message); batch.finishedAt = new Date().toISOString(); persist(); });
    return batchStatus();
  } catch (e) {
    console.error('[leadgen-batch] resume check failed:', e.message);
    return null;
  }
}

module.exports = { startBatch, cancelBatch, batchStatus, resumeOnBoot };
