...
const { makeSoloTeam } = require('./soloTeam');
...
async function onboardCustomer({
  ...
  try {
    ...
    // 5b) The team is the owner alone (Peter, 2026-09-15): the seed's sample
    //     people never reach a real owner's wizard; the owner is the one
    //     bookable person until a colleague is added. Best-effort: a failure
    //     leaves the sample team in place (the wizard can still clear it).
    try {
      await makeSoloTeam(site.siteId);
    } catch (e) {
      console.error('[onboard] solo team:', e.message);
    }

    // 5c) AI Front desk on by default (Peter, 2026-09-16): the chat assistant is
    //     part of the offer, the wizard's radio shows Yes and the owner can
    //     switch it off there or under Website → Front desk.
    try {
      const { data: row } = await supabase.from('site_theme_settings').select('site_id, metadata').eq('site_id', site.siteId).maybeSingle();
      const meta = { ...((row?.metadata && typeof row.metadata === 'object') ? row.metadata : {}), frontdesk_enabled: true };
      const { error } = row
        ? await supabase.from('site_theme_settings').update({ metadata: meta }).eq('site_id', site.siteId)
        : await supabase.from('site_theme_settings').insert({ site_id: site.siteId, metadata: meta });
      if (error) throw new Error(error.message);
    } catch (e) {
      console.error('[onboard] front desk default:', e.message);
    }
