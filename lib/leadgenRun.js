...
async function startRun(user, body = {}) {
  const {
    ...
    search_query,
    max_results  = 30,
    min_score    = 7, // default since 2026-09-18 (Peter): established shops and up; lower scores stay in Places
    dry_run      = false,
  } = body;
  ...
  const minScore   = Math.min(Math.max(parseInt(min_score, 10) || 7, 1), 10);     // clamp 1–10
