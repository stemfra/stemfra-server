...
async function findBusiness({ name, where, country = 'US' }) {
  const items = await searchPlaces({ name, where, country });
  return items.map(toCandidate);
}

/** The raw Apify place items for a name near a place (the lead refresh keeps the
 *  full record, the wizard shapes it with toCandidate). */
async function searchPlaces({ name, where, country = 'US', max = MAX_PLACES }) {
  ...
  try {
    ...
    return items.filter((p) => p && p.title);
  } finally { clearTimeout(t); }
}

module.exports = { findBusiness, searchPlaces, hoursFromGoogle, toCandidate, configured };
