...
const { verticalOfLead } = require('./leadVertical');

// leads.vertical (the canonical column) → the platform's vertical slug. The legacy
// `template_slug` only knows five design-gallery names ('wellness', 'fitness', 'studio' map to
// NO platform vertical), so a spa, massage, CrossFit or yoga lead used to get the generic
// templates page with no demo and no hero (found 2026-09-17, minutes before a spa's email).
const LEAD_TO_PLATFORM = { barbershop: 'barbershops', beauty_salon: 'salons', crossfit: 'crossfit', yoga_pilates: 'yoga_pilates', massage: 'massage', spa: 'spa' };
...
async function resolveClaimOffer(lead) {
  const vertical = LEAD_TO_PLATFORM[verticalOfLead(lead)] || resolveVerticalSlug(lead.template_slug || lead.service_type || 'barbershops');
