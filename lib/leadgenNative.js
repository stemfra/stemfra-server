...
const PLATFORMS = [
  ...
  ['other', /janeapp|jane\.app|massagebook|treatwell|wellnessliving|momence|zenoti|booker\.com|genbook|simplybook|calendly/i],
];
...
function candidateOf(p) {
  ...
  return {
    ...
    email: p.emails?.[0] || p.contactDetails?.emails?.[0] || '',
    website: p.website || '',
    city: p.city || '',
...
function leadRow(ai, c, { runCountry, runId }) {
  ...
  return {
    ...
    suggested_channel: ai.suggested_channel || null,
...
async function runNative({ runId, payload, close, dryRun = false }) {
  ...
  try {
    ...
    await progress({ stage: 'scoring', apify_run_id: apifyRunId, location, candidates: fresh.length });
    ...
    const scored = await pool(fresh, SCORE_CONCURRENCY, async (c) => {
      try { return { c, ai: await score(c, ctx) }; } catch (e) { return { c, error: e.message }; }
    });
    ...
    if (!dryRun) {
      await progress({ stage: 'inserting', apify_run_id: apifyRunId, location, kept: keep.length });
...
module.exports = { runNative, volumeOf, configured, candidateOf, bookingPlatformOf, hasOwnSite, locationOf, systemPrompt };
