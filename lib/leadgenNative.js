...
async function runNative({ runId, payload, close, dryRun = false }) {
  ...
  try {
    ...
    for (const { c, ai, error } of scored) {
      ...
      if (ai.is_relevant !== true) { counts.not_relevant++; decide(c, 'score', 'not relevant', 'not_relevant', detail, place); continue; }
...
async function draftForLead(leadId, { template = null } = {}) {
  ...
  let tpl = template;
  if (!tpl) {
    const { data } = await supabase.from('email_templates').select('subject, body').eq('code', 'A1').eq('is_active', true).maybeSingle();
    tpl = data || null;
  }
...
async function promotePlace(placeId, { userId = null } = {}) {
  ...
  const row = { ...leadRow(ai, c, { runCountry: c.country, runId: pl.last_run_id }), review_status: 'approved' };
...
module.exports = { runNative, draftForLead, promotePlace, savePlaces, ownerEmail, volumeOf, configured, candidateOf, bookingPlatformOf, hasOwnSite, locationOf, systemPrompt };
