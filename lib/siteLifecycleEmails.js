...
async function loadSite(siteId) {
  const { data, error } = await supabase
    .from('sites')
    .select('id, subdomain, custom_domain, metadata, company:companies(name), owner:contacts!sites_owner_contact_id_fkey(email, first_name, last_name, full_name, auth_user_id)')
    .eq('id', siteId)
    .single();
...
async function sendSiteLiveEmail(siteId, { to = null } = {}) {
  ...
  if (!who) return false;
  // "Add your clients" is a one-click magic link into the Clients page with
  // the import panel open (falls back to the plain URL).
  const clientsPath = '/customers?import=1';
  const clientsUrl = (await cmsMagicLink(site.owner?.auth_user_id, clientsPath)) || `${CMS_URL}${clientsPath}`;
