...
// ─── copySiteParts (2026-09-12, the YouTube "Reuse details" idea) ───────────
// A new site is provisioned from the vertical's seed as usual; then the parts
// the owner ticked are REPLACED with copies from one of their own sites:
//   brand    site_theme_settings (+ template_id when the vertical matches)
//   services categories + services (+ team↔service links when team is copied too)
//   team     team members + availability rules
//   hours    sites.business_hours + time_zone
//   content  pages + sections + testimonials (About, FAQ, reviews)
// Untouched parts keep the seed's demo data, so the wizard still has something
// to confirm-or-edit. Text is renamed from the source brand to the new one.
const SITE_PARTS = ['brand', 'services', 'team', 'hours', 'content'];

async function deleteRows(table, siteId) {
  const { error } = await supabase.from(table).delete().eq('site_id', siteId);
  if (error) throw new Error(`delete ${table}: ${error.message}`);
}

async function copySiteParts(sourceSiteId, newSiteId, parts, { rename = null, renameTo = null } = {}) {
  const want = new Set((parts || []).filter((p) => SITE_PARTS.includes(p)));
  if (!want.size) return { copied: [] };
  if (!rename && renameTo) {
    const srcName = await sourceBrandName(sourceSiteId);
    if (srcName && srcName !== renameTo) rename = { from: srcName, to: renameTo };
  }
  const R = rename ? makeRenamer(rename.from, rename.to) : null;
  const rn = (row, keys) => (R ? Object.fromEntries(Object.entries(row).map(([k, v]) => [k, keys.includes(k) ? R(v) : v])) : row);
  const copied = [];
  ...
  if (want.has('content')) {
    for (const t of ['site_sections', 'site_pages', 'site_testimonials']) await deleteRows(t, newSiteId);
    const srcPages = await fetchRows('site_pages', sourceSiteId);
    const pageIdMap = {};
    await insertRows('site_pages', srcPages.map((p) => { const id = randomUUID(); pageIdMap[p.id] = id; return rn({ ...omit(p, TIMESTAMPS), id, site_id: newSiteId }, ['title', 'meta_title', 'meta_description']); }));
    const srcSections = await fetchRows('site_sections', sourceSiteId);
    await insertRows('site_sections', srcSections.map((s) => rn({ ...omit(s, TIMESTAMPS), id: randomUUID(), site_id: newSiteId, page_id: pageIdMap[s.page_id] }, ['content'])).filter((s) => s.page_id));
    const srcTest = await fetchRows('site_testimonials', sourceSiteId);
    await insertRows('site_testimonials', srcTest.map((t) => rn({ ...omit(t, TIMESTAMPS), id: randomUUID(), site_id: newSiteId, author_photo_media_id: null }, ['quote', 'author_role'])));
    copied.push('content');
  }
  ...
  if (want.has('services')) {
    for (const t of ['site_team_service_links', 'site_services', 'site_service_categories']) await deleteRows(t, newSiteId);
    const srcCats = await fetchRows('site_service_categories', sourceSiteId);
    const catIdMap = {};
    await insertRows('site_service_categories', srcCats.map((c) => { const id = randomUUID(); catIdMap[c.id] = id; return rn({ ...omit(c, TIMESTAMPS), id, site_id: newSiteId }, ['name', 'description']); }));
    const srcServices = await fetchRows('site_services', sourceSiteId);
    await insertRows('site_services', srcServices.map((sv) => { const id = randomUUID(); serviceIdMap[sv.id] = id; return rn({ ...omit(sv, TIMESTAMPS), id, site_id: newSiteId, category_id: sv.category_id ? catIdMap[sv.category_id] || null : null, photo_media_id: null }, ['name', 'description']); }));
    copied.push('services');
  }
  if (want.has('team')) {
    for (const t of ['site_team_service_links', 'site_availability_rules', 'site_team_members']) await deleteRows(t, newSiteId);
    const srcTeam = await fetchRows('site_team_members', sourceSiteId);
    await insertRows('site_team_members', srcTeam.map((tm) => { const id = randomUUID(); teamIdMap[tm.id] = id; return rn({ ...omit(tm, TIMESTAMPS), id, site_id: newSiteId, photo_media_id: null }, ['bio', 'role']); }));
    const srcAvail = await fetchRows('site_availability_rules', sourceSiteId);
    await insertRows('site_availability_rules', srcAvail.map((a) => ({ ...omit(a, TIMESTAMPS), id: randomUUID(), site_id: newSiteId, team_member_id: teamIdMap[a.team_member_id] })).filter((a) => a.team_member_id));
    copied.push('team');
  }
  if (want.has('services') && want.has('team')) {
    const srcLinks = await fetchRows('site_team_service_links', sourceSiteId);
    await insertRows('site_team_service_links', srcLinks.map((l) => ({ id: randomUUID(), site_id: newSiteId, team_member_id: teamIdMap[l.team_member_id], service_id: serviceIdMap[l.service_id] })).filter((l) => l.team_member_id && l.service_id));
  }

  if (want.has('brand') || want.has('hours')) {
    const { data: src, error } = await supabase.from('sites').select('template_id, vertical_id, business_hours, time_zone').eq('id', sourceSiteId).single();
    if (error || !src) throw new Error(`copy parts: source site not found`);
    const { data: dst } = await supabase.from('sites').select('vertical_id').eq('id', newSiteId).single();
    const patch = {};
    if (want.has('hours')) { patch.business_hours = src.business_hours; patch.time_zone = src.time_zone; copied.push('hours'); }
    if (want.has('brand')) {
      if (dst && dst.vertical_id === src.vertical_id && src.template_id) patch.template_id = src.template_id;
      await deleteRows('site_theme_settings', newSiteId);
      const srcTheme = await fetchRows('site_theme_settings', sourceSiteId);
      // footer text and the SEO defaults in metadata carry the brand name: rename them too.
      await insertRows('site_theme_settings', srcTheme.map((ts) => rn({ ...omit(ts, TIMESTAMPS), id: randomUUID(), site_id: newSiteId }, ['footer_text', 'metadata'])));
      copied.push('brand');
    }
    if (Object.keys(patch).length) {
      const { error: upErr } = await supabase.from('sites').update(patch).eq('id', newSiteId);
      if (upErr) throw new Error(`copy parts: sites update: ${upErr.message}`);
    }
  }
  return { copied };
}
