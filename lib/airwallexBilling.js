...
// The billing customer's phone (Airwallex `phone_number`, optional, E.164 in
// their example): the owner contact's phone when it parses, else omitted so a
// malformed number can never fail the per-invoice customer patch (Peter,
// 2026-09-14). Airwallex records it; nothing in their docs says it prints.
function phoneFrom(contact) {
  try {
    const { parsePhoneNumberFromString } = require('libphonenumber-js');
    const p = contact?.phone ? parsePhoneNumberFromString(String(contact.phone), isoCountry(contact.country) || 'US') : null;
...
async function ensureBillingCustomer(ctx) {
  ...
  if (ownerContactId) {
    const { data: c } = await supabase.from('contacts')
      .select('billing_profile, country, state, phone').eq('id', ownerContactId).maybeSingle();
    contact = c || null;
  }
  const profile = contact?.billing_profile || {};
  const address = addressFrom(contact);
  const phone = phoneFrom(contact);
  ...
  if (profile.awx_customer_id) {
    ...
    const patch = { ...(address ? { address } : {}), ...(nickname ? { nickname } : {}), ...(phone ? { phone_number: phone } : {}) };
