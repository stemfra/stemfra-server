...
// `to` overrides the recipient for STAFF TEST SENDS only (Peter's own mailboxes).
async function sendInvoiceEmail(chargeId, { to = null } = {}) {
  try {
    const c = await loadChargeContext(chargeId);
    if (!c || !(to || c.ownerEmail)) return false;
