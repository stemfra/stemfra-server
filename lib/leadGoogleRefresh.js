...
const placesLookup = require('./placesLookup');
 ...
 */
async function refreshLeadFromGoogle(leadId, { dryRun = false, actor = null, force = false } = {}) {
  ...
  // OUR OWN DATA FIRST (2026-09-17): lead-gen keeps every scraped place, so most refreshes are
  // free. `force` skips this and asks Google again (newer numbers). A stale row is not used.
  if (!force) {
    const row = (lead.source_detail && await placesLookup.placeByKey({ url: lead.source_detail }))
      || (await placesLookup.placesByName({ name: lead.company_name, where, phone: lead.phone }))[0] || null;
    if (row && row.record && row.record.placeId && placesLookup.isFresh(row)) {
      const record = { ...row.record, refreshed_from: `stemfra places ${new Date().toISOString().slice(0, 10)}` };
      const signals = placesLookup.signalsFromRecord(row.record);
      const readiness = row.readiness || (row.record.claimed ? (signals.length > 1 ? 'modern' : 'middle') : 'old_school');
      if (dryRun) return { status: 'dry_run', title: record.title, readiness, signals, where, source: 'places' };
      const patch = { raw_signal: JSON.stringify(record), readiness, qualification: { ...(lead.qualification || {}), readiness_signals: signals } };
      if (!lead.phone && (record.phoneUnformatted || record.phone)) patch.phone = record.phoneUnformatted || record.phone;
      if (!lead.source_detail && record.url) patch.source_detail = record.url;
      const { error: pErr } = await supabase.from('leads').update(patch).eq('id', leadId);
      ...
      await logActivity({
        ...
        details: { readiness, signals, reviews: record.reviewsCount ?? null, claimed: record.claimed, source: 'places' },
      });
      return { status: 'updated', title: record.title, readiness, signals, where, source: 'places' };
    }
  }
  ...
  return { status: 'updated', title: record.title, readiness, signals, where, source: 'google' };
}
